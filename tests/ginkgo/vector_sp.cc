...
// Tests the GinkgoWrappers::Vector interface for vector space operations that rely on reductions
...
TEST(can_compute_add_and_dot){
  GinkgoWrappers::Vector<double> v({1, 2, 3, 4}, exec);
  GinkgoWrappers::Vector<double> w({4, 3, 2, 1}, exec);
  GinkgoWrappers::Vector<double> u({2, 1, 4, 3}, exec);
...
TEST(can_compute_all_zero){
  ...
  {
    GinkgoWrappers::Vector<double> v({0, 0, 0}, exec);
